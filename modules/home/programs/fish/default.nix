    ...
    functions =
      {
        ...
        fish_prompt = ''
          ...
          set icon 󰉋
          set color ${opts.theme.col.brightBlue.hex}
          if set -q IN_NIX_SHELL
            set icon 󱄅
            set color ${opts.theme.col.brightMagenta.hex}
          end

          echo -n "$(set_color $color)$icon$(set_color normal)$(set_color -i) $cwd[-2]$(set_color $color)$cwd[-1]$(set_color normal) ) "
        '';
        fish_right_prompt = ''
