...
#       [
#         astal.io
#         (writers.writeFishBin "reload_ags" ''
#           ags quit
#           hyprctl dispatch exec ags run
#         '')
#         dart-sass
#       ]
#       ++ (lib.optionals opts.widgets.visualizer.enable [libcava]);
#     programs.ags = {
#       enable = true;
...
#           # gtk3
#         ]
#         ++ (lib.optionals opts.widgets.visualizer.enable [cava]));
#     };
#     xdg.configFile = let
...
#           visualizer = {
#             enable = opts.widgets.visualizer.enable;
#             bars = opts.widgets.visualizer.bars;
#           };
#         };
#       };
#     };
...
in {
  config = lib.mkIf (opts.widgets.enable) {
    home.packages = [inputs.nigits.packages.${const.system}.default];
